...
type: blog
date: 2026-08-09
talkUrl: /talks/out-of-order-streaming
coverUrl: https://storage.googleapis.com/julianburr-me-assets/blog-covers/out-of-order-streaming.png
---

_This is a write-up of a [conference talk given at multiple conferences](/my-work/talks/out-of-order-streaming), including [ReactSummit US 2024](https://gitnation.com/contents/lets-build-suspense), [Øredev 2025](https://www.youtube.com/watch?v=TJ2JOa0w6QY) and WeAreDevelopers World Congress 2026._
...
Also, this is not one of those posts that tells you that you have to user server-side rendering and streaming, and if you don't you are wrong. Like with everything and live, and especially in software engineering, there is always more nuance to things. Instead, I hope this post gives you a bit more details and understandings how these new technologies and techniques work and what problem we are trying to solve with them, so you can make a more informed decision for yourself whether or not this seems useful for your particular product, project or team.
...
::demo{id="renderingTimeline" configKey="html"}
...
::demo{id="renderingTimeline" configKey="jquery"}
...
::demo{id="renderingTimeline" configKey="spa"}
...
::demo{id="renderingTimeline" configKey="ssr"}
...
::demo{id="renderingTimeline" configKey="serverComponents"}
...
This is creating a fairly basic NodeJS server, with an endpoint that returns our app. I'm using JSX here, but again you don't have to, it's mainly helpful to keep things readable and concise here for the purpose of this demo.
...
It's not just a function, it's also asynchronous. For the sake of this demo, we treat all of our components as "server components". As we've explored before means they are only rendered once (on the server). This also means we don't need to worry about any client-side bundle here, which is great.
...
To fix that, let's introduce streaming. HTTP streaming has been around forever, basically since the web started. The idea is simple: when the browser request comes in, the server responds immediately, but leaves the connection open, and then keeps sending more stuff as it becomes available. Once everything is sent, the server closes the connection, so the browser knows the interaction is over.
...
And we want to render said fallback until the children of the Suspense component are all ready to be rendered. We also want [the boundary to be identifiable[^2]]() (you'll see what for in a second), so we give each one a unique ID.
...
### The future: No Javascript at all

I just said above that to swap out the suspense loading state for the final content we need a little bit of Javascript. That's hopefully not true for much longer!

There is active development from the Chrome team on what they call ["declarative partial updates"](https://developer.chrome.com/blog/declarative-partial-updates). This includes a range of different API proposals that aim to make the web work more seamless with modern content delivery approaches, and out-of-order streaming is explicitly one of them.

In particular, they propose extended functionality for the `<template>` tag, that would allow us to define the whole placeholder behaviour and final content being streamed in later with pure HTML.

To achieve this, they introduce the idea of markers:

```html
<div><?marker name="some-placeholder"></div>

...

<template for="some-placeholder">
  Here is some <em>HTML content</em>!
</template>
```

In the example code above the marker would define where the final content will eventually go. We would then simply reference the marker name through the `for` attribute on the template tag to associate the streamed content with it.

To actually inject loading state that gets replaced once the template tag is rendered, they introduce `start` and `end` markers instead:
...
This is obviously still _very_ experimental, but you can see that it has been heavily inspired by the way React and [Suspense on the server[^3]](), as well as other modern full-stack frameworks, already opperate. Which makes me personally very optimistic that this proposal has good chances of making it into the other browsers, hopefully in the not-so-far-away future 😊
...
**What might the future bring?** I personally hope things like out-of-order streaming, server components, "suspense"-like loading boundaries, etc., will all get adopted more and more into the core of either our application frameworks, especially if the core platform enables it more e.g. by extending the functionality of the template tag. Either way, the goal should be that developers don't need to know about all this or even think about it; it's all hidden away behind a low-level abstraction layer. Which also means it would be easy to change and adapt as our ideas and technologies evolve.

---

[^1]: **Side quest:** Without spending too long on the nostalgia here, I still want to highlight what a massive role jQuery has played for Javascript to become what it is today. Not only did it help with the aforementioned cross-browser compatibility, it also added many utilities that made common tasks massively more developer-friendly (e.g. event listeners, selecting DOM elements, manipulating the DOM, etc.).

[^2]: **Side note:** A bit of random CSS trivia here. If you ever need a div (e.g. to be able to target the DOM element itself, but maybe also for styling purposes, etc.) but you don't want it to affect the layout of the current parent node, you can use display: contents. This tells the CSS layout engine to ignore this div for layout purposes. E.g. if the container is using flex or grid, the "contents" div will be ignored for the flex or div layout. Instead, the engine will look at the children to construct the layout. Pretty cool!

[^3]:
    Just to be clear and avoid being misleading in any way: what we've been looking at in this blog post is a very simplified version of how Suspense works in principle. The actual implementation details in React are actually quite different, because React is not only trying to stream in the plain HTML, but also a lot of additional meta information about the React components that gets passed through in a JSON-like format.

    If you want to dive more into that I highly recommend reading [the original React RFC and related resources](https://github.com/reactjs/rfcs/blob/main/text/0213-suspense-in-react-18.md) on it.
